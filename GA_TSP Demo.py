...
class TSPGUI(tk.Tk):
    ...
    def process(self):
        ...
        if not start_city or not selected_cities:
            messagebox.showerror("Lỗi", "Vui lòng chọn thành phố bắt đầu và ít nhất 1 thành phố đi qua.")
            self.status_var.set("Lỗi: chưa chọn đủ dữ liệu")
        ...
        try:
            ...
            # Đưa start_city về đầu tuyến đường và quay lại điểm xuất phát
            if start_city in route_names_raw:
                start_idx = route_names_raw.index(start_city)
                route_names = route_names_raw[start_idx:] + route_names_raw[:start_idx]
            else:
                route_names = route_names_raw
            # Thêm start_city vào cuối để quay về điểm xuất phát
            if route_names[-1] != start_city:
                route_names.append(start_city)
